...
export interface Usuario { id: number; nombre: string; email: string; rol: Rol; activo: boolean; fechaCreacion: string; }
...
export interface OrdenCompra { id: number; proveedor: Proveedor; usuario: Usuario; fechaCreacion: string; fechaEsperada: string; estado: EstadoOrden; observaciones: string; total: number; detalles: DetalleOrden[]; }
export interface MovimientoInventario { id: number; producto: Producto; tipoMovimiento: string; cantidad: number; usuarioResponsable: Usuario; referencia: string; fecha: string; }
